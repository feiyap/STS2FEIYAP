...
- 模板 Wiki（以 Rider 为主线）：[中文首页](https://github.com/alkaid616/Feiyap/wiki/Home) | [English Home](https://github.com/alkaid616/Feiyap/wiki/Home-EN)。
...
`dotnet new ritsulibmod -n MyMod` 会生成名为 `MyMod` 的工程，并把模板中的 `Feiyap`、示例类名、资源文件名、资源目录、manifest 名称、namespace 和本地化 id 同步替换成新名称。

### 方式 B：手动复制模板

1. 复制整个目录并改名为你的 Mod 名称。
2. 修改 `Feiyap.json` 里的 `id`、`name`、`author`、`description`。
3. 修改 `FeiyapCode/Entry.cs` 里的 `ModId`。
4. 如需彻底改名，同时修改 `.csproj`、`.sln`、`project.godot` 的项目名和命名空间。
5. 把资源目录 `Feiyap/` 改成你的 `ModId`，并同步更新代码中的 `Entry.ResPath` 相关路径。
...
>
> `Feiyap.json` 中 `dependencies[STS2-RitsuLib].version` **必须**与 `.csproj` 里 `STS2.RitsuLib` 包实际编译使用的版本一致。两者互相独立、**不会自动同步**——不对齐会让玩家通过 manifest 校验后在运行时崩溃，或在 RitsuLib 已满足条件时被错误拒绝。详细步骤见下方 [发布前 checklist：版本对齐](#发布前-checklist版本对齐)。
...
> **`.csproj` 里的 `PackageReference` 只控制编译时拉取；`Feiyap.json` 的 `dependencies` 是游戏加载器在运行时校验的。两者互相独立，不会自动同步。**
...
2. 把该版本填入 `Feiyap.json` 的 `dependencies[STS2-RitsuLib].version`。
...
| 命令 | 行为 |
|---|---|
| `dotnet build .\Feiyap.csproj` | 完整构建：编译 + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**：复制 dll 和 manifest 到游戏的 `mods/Feiyap` 目录。
...
`res://Feiyap/...` 是 Godot/PCK 内的资源路径，对应仓库里的 `Feiyap/` 资源目录，**不是 C# namespace**。通过 NuGet 模板创建项目时，这些目录名、文件名和 namespace 会按新 Mod 名同步替换。
...
| 初始遗物 | `ShuiLian`（`LianXinShouYue` 为欧罗巴斯之触升级形态） |
...
- `relics/ShuiLian.png`、`relics/LianXinShouYue.png`：初始遗物图标。
...
| 场景文件 | 用途 | 占位结构 |
|---|---|---|
| `Feiyap_character.tscn` | 战斗人物 | `%Visuals`、`%Bounds`、`%IntentPos`、`%CenterPos`、`%TalkPos` |
| `Feiyap_energy_counter.tscn` | 能量表盘 | `%EnergyVfxBack`、`%Layers`、`%RotationLayers`、`%EnergyVfxFront`、`Label` |
| `Feiyap_merchant.tscn` | 商店人物 | — |
| `Feiyap_rest_site.tscn` | 火堆人物 | `%ControlRoot`、`%SelectionReticle`、`%Hitbox`、`%ThoughtBubbleRight`、`%ThoughtBubbleLeft` |
| `Feiyap_character_select_bg.tscn` | 角色选择背景 | — |

这些资源只用于保证模板可见、可替换，不追求原版动画效果。复制模板后替换为自己的素材即可；如果改了路径，同步更新对应 `AssetProfile`。

## Manifest 格式

`Feiyap.json` 是 Mod 的清单文件，游戏加载器在启动时读取它来识别 Mod、检查依赖、决定是否加载。完整示例：
