...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/Feiyap/wiki/Home) | [English Home](https://github.com/alkaid616/Feiyap/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `Feiyap`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `Feiyap.json` and update `id`, `name`, `author`, and `description`.
3. Edit `FeiyapCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `Feiyap/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
>
> `dependencies[STS2-RitsuLib].version` in `Feiyap.json` **must exactly match** the `STS2.RitsuLib` version your `.csproj` actually compiles against. The two are independent and **never auto-synced** — mismatches let players pass the manifest check and crash at runtime, or get wrongly rejected when their RitsuLib would have worked. See [Pre-release checklist: version alignment](#pre-release-checklist-version-alignment) below for the step-by-step procedure.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `Feiyap.json` is what the game loader checks at runtime. The two are independent and never auto-synced.**
...
2. Write that version into `dependencies[STS2-RitsuLib].version` in `Feiyap.json`.
...
| Command | Behavior |
|---|---|
| `dotnet build .\Feiyap.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/Feiyap` directory.
...
Feiyap/
├── FeiyapCode/   # C# source
├── Feiyap/       # Godot resources, localization, and placeholder scenes
├── Feiyap.csproj
├── Feiyap.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://Feiyap/...` is the Godot/PCK resource path, mapping to the repository resource directory `Feiyap/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Starter deck | 4 × `FeiyapStrike`, 4 × `FeiyapDefend`, 1 × `FeiyapRelic` |
...
- `relics/FeiyapRelic.png`: sample relic icon.
...
|---|---|---|
| `Feiyap_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `Feiyap_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `Feiyap_merchant.tscn` | Merchant | — |
| `Feiyap_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `Feiyap_character_select_bg.tscn` | Character select background | — |
...
`Feiyap.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
